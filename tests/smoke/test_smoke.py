"""Basic smoke checks for the connected ESP32-S3 DUT."""

import re

import pytest

from drivers.device_driver import DeviceDriver
from tests.constants import (
    DISTANCE_LOG_MARKER,
    DISTANCE_UNIT,
    LED_ON_COMMAND_ECHO,
    MIN_VALID_DISTANCE_CM,
    STATUS_COMPLETION_MARKER,
    STATUS_MARKER,
)
from tests.helpers import normalized_response_to_string

pytestmark = pytest.mark.smoke


def test_status_command_works(clean_device: DeviceDriver) -> None:
    response = clean_device.get_status()
    response_text = normalized_response_to_string(response)

    assert response, "The status command returned no response"
    assert STATUS_MARKER.casefold() in response_text, "Status marker was not found"
    assert STATUS_COMPLETION_MARKER.casefold() in response_text, "Status did not complete"


def test_distance_command_returns_measurement(clean_device: DeviceDriver) -> None:
    response = clean_device.get_distance()
    response_text = normalized_response_to_string(response)
    match = re.search(
        rf"{re.escape(DISTANCE_LOG_MARKER.casefold())}.*?(\d+(?:\.\d+)?)\s*{DISTANCE_UNIT}",
        response_text,
    )

    assert match is not None, f"Numeric distance was not found: {response_text!r}"
    assert float(match.group(1)) >= MIN_VALID_DISTANCE_CM


def test_led_command_works(clean_device: DeviceDriver) -> None:
    response = clean_device.led_on()
    response_text = normalized_response_to_string(response)

    assert LED_ON_COMMAND_ECHO.casefold() in response_text, (
        f"The device did not echo the LED command: {response_text!r}"
    )
